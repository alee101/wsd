...
def similarity_by_infocontent(sense1, sense2, option):
  ...
  info_contents = ['ic-bnc-add1.dat', 'ic-bnc-resnik-add1.dat', 
                   ...
                   'ic-brown-resnik.dat', 'is.c-brown.dat', 
...
def sim(sense1, sense2, option="res"):
...
def max_similarity(context_sentence, ambiguous_word, option="path", 
  ...
  for i in wn.synsets(ambiguous_word):
    if pos and pos != str(i.pos()):
      continue
    result[i] = sum(max([sim(i,k,option) for k in wn.synsets(j)]+[0]) for j in context_sentence)
  ...
  print result
  print result[0][1]
  if best: return result[0][1].definition;
  return result
...
n = len(brown.sents()) - 1
for i in brown.sents()[:n]:
  j = ' '.join(i)
  'print j'
  'print i[3]'
  'about = max_similarity(j, i[3])'
  'print (about)'
'print (brown.sents())'
...
'''
ftp://learning.cs.utoronto.ca/public_html/public_html/pub/gh/Budanitsky+Hirst-2001.pdf

maybe: http://aclweb.org/anthology//P/P11/P11-1148.pdf
'''