...
\begin{document}
	...
	\section{Algorithms}
	 ...
	 We implemented a pipeline each with a different type of algorithm in order to do 
	 word sense disambiguation. The first step is a part-of-speech tagging step that uses
	 bigram models in order to determine what the usual part of speech is in a certain location. 
	 We use this approach for a subset of data that has high predictability. 

	 Previously, we had a step involving semantic knowledge approaches, but this had low accuracy and we discarded it from the pipeline. We still detail the methods below. 

 	 For the rest of the data, we applied supervised LSA, a novel implementation of the simple LSA algorithm with some ideas drawn from more sophisticated LSA approaches. 
